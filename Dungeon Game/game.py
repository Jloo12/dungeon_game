...
from resource_manage.colors import *
from resource_manage.image_manage import *
from resource_manage.game_controller import *
from resource_manage.basic_var import *

from render_game.render_map import maze, MAZE_H, MAZE_W, dungeon, DUNGEON_H, DUNGEON_W
...
def draw_dungeon(bg, fnt): #繪製地下城
    ...
    bg.blit(imgDark, [0, 0]) #在四個角落配置暗沈的圖片
...
def put_event(): #於地板配置道具
    ...
    while True:
        ...
        if dungeon[y][x] == 0:
            for ry in range(-1, 2): #將樓梯周圍的空間設為地板
                for rx in range(-1, 2):
                    dungeon[y + ry][x + rx] = 0
            dungeon[y][x] = 3
            break
...
def move_player(key): #主角的移動
    global idx, tmr, pl_x, pl_y, pl_d, pl_a, pl_life, food, \
        potion, blazegem, treasure
...
def draw_text(bg, txt, x, y, fnt, col): #顯告套用陰影效果的文字
...
def main(): #主要處理
    ...
    while True:
        ...
        if idx == 0: #標題畫面
            ...
            if fl_max >= 2:
                draw_text(screen, "You Reach floor {}".format(fl_max), 300 , 400, font, CYAN)
            ...
            if tmr <= 30:
                PL_TURN = [2, 4, 0, 6]
                pl_a = PL_TURN[tmr % 4]
                if tmr == 30: pl_a = 8 #主角倒地畫面
                draw_dungeon(screen, fontS)
            elif tmr == 31:
                se[3].play()
                draw_text(screen, "you died", 360, 240, font, RED)
                draw_text(screen, "Game over", 360, 360, font, RED)
            elif tmr == 100:
                idx = 0
                tmr = 0
            ...
            if tmr == 11:
                ...
                if emy_life <= 0:
                    ...
                    idx = 16
                    tmr = 0
            if tmr == 16: #JJJ 這裡晚點測試tmr == 1會有什麼運行問題 ＪＪＪ
                idx = 13
                tmr = 0
        ...
        draw_text(screen, "[M]ute" + mute , 740, 80, fontS, WHITE)
